...
import { HERO_ENTERPRISE_BG, PRODUCT_BG_IMAGES, CALENDEX_SCHEDULE_MOCKUP_IMAGE } from '../data/productImages';
...
export const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  const [heroBgOpacity, setHeroBgOpacity] = useState<number>(65);
  const [selectedHeroBg, setSelectedHeroBg] = useState<string>('calendex');
  const [autoPlayBg, setAutoPlayBg] = useState<boolean>(true);
  const [isKenBurnsAnimated, setIsKenBurnsAnimated] = useState<boolean>(true);
  ...
  // Auto slideshow sequence for background image if enabled
  useEffect(() => {
    if (!autoPlayBg) return;
  ...
  return (
    <section id="hero" className={`relative pt-32 pb-24 md:pt-40 md:pb-32 overflow-hidden ${isDark ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <img 
          ...
          className={`w-full h-full object-cover transition-all duration-1000 transform ${
            isKenBurnsAnimated ? 'animate-kenburns' : ''
          }`}
        ...
        <div className="absolute top-0 left-1/4 w-96 h-full bg-gradient-to-b from-cyan-400/20 via-transparent to-purple-500/20 pointer-events-none animate-beam blur-xl" />
        <div className="absolute top-0 right-1/4 w-80 h-full bg-gradient-to-b from-indigo-500/20 via-transparent to-cyan-400/20 pointer-events-none animate-beam blur-xl" style={{ animationDelay: '4s' }} />
      </div>
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1000px] h-[600px] bg-gradient-to-tr from-indigo-600/25 via-purple-600/20 to-cyan-500/25 rounded-full blur-[120px] animate-pulse-glow" />
      </div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Background Image Controls Bar in Hero */}
        <div className="flex items-center justify-center gap-3 mb-8 flex-wrap">
          <div className={`px-4 py-2 rounded-2xl ${isDark ? 'bg-slate-900/90 border-slate-800 text-slate-300' : 'bg-white/90 border-slate-200 text-slate-700 shadow-md'} border text-xs font-bold flex items-center gap-3 backdrop-blur-md shadow-xl`}>
            <div className="flex items-center gap-1.5 text-cyan-500 font-extrabold">
              <Sparkles className="w-4 h-4 animate-spin text-cyan-400" style={{ animationDuration: '6s' }} />
              <span>Animated Hero Background:</span>
            </div>
            
            <div className="flex items-center gap-1 overflow-x-auto max-w-xs sm:max-w-none pb-1 sm:pb-0">
              {Object.entries(heroBgOptions).map(([key, opt]) => (
                <button
                  key={key}
                  onClick={() => {
                    setSelectedHeroBg(key);
                    setAutoPlayBg(false);
                  }}
            ...
            <div className={`flex items-center gap-1.5 border-l ${isDark ? 'border-slate-800' : 'border-slate-200'} pl-3`}>
              <SlidersHorizontal className="w-3.5 h-3.5 text-purple-400" />
              <span className={`text-[11px] whitespace-nowrap ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{heroBgOpacity}%</span>
              <input 
                type="range" 
                min="10" 
                max="85" 
        ...
        <div className="text-center max-w-4xl mx-auto space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
            ...
            <button
              onClick={() => onOpenDemo()}
              className="w-full sm:w-auto px-8 py-4 rounded-xl text-base font-bold bg-gradient-to-r from-indigo-600 via-purple-600 to-cyan-500 text-white shadow-xl shadow-indigo-500/25 hover:shadow-indigo-500/40 hover:scale-[1.02] active:scale-95 transition-all duration-200 flex items-center justify-center gap-3 group"
            >
            ...
            <button
              ...
              className={`w-full sm:w-auto px-8 py-4 rounded-xl text-base font-bold border transition-all duration-200 flex items-center justify-center gap-2 ${
                isDark 
                  ...
                  : 'border-slate-300 bg-white text-slate-900 hover:bg-slate-100 shadow-sm'
              }`}
            >
        ...
        <div className="mt-16 lg:mt-20 max-w-5xl mx-auto relative">
          ...
          <div className={`rounded-3xl p-6 sm:p-10 border ${isDark ? 'border-slate-800/80 bg-slate-950/85' : 'border-slate-200 bg-white shadow-xl'} backdrop-blur-xl relative overflow-hidden transition-all duration-300 shadow-2xl`}>
            ...
            <div className={`flex flex-col sm:flex-row items-start sm:items-center justify-between pb-6 mb-8 border-b ${isDark ? 'border-slate-800/50' : 'border-slate-200'} gap-4`}>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className={`hidden md:flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border ${isDark ? 'text-slate-300 bg-slate-900/80 border-slate-800' : 'text-slate-700 bg-slate-100 border-slate-200'}`}>
                  ...
                  <span className="font-bold text-cyan-500 capitalize">{activeNode}</span>
                </div>
              </div>
            </div>
            ...
            <div className="relative min-h-[420px] sm:min-h-[460px] flex items-center justify-center">
              ...
              >
                <div className="relative flex items-center justify-center w-28 h-28 sm:w-36 sm:h-36 rounded-full bg-slate-950 border-2 border-cyan-400/80 shadow-2xl shadow-indigo-500/50 p-2">
                  <div className="absolute inset-0 rounded-full bg-gradient-to-tr from-indigo-600/30 via-purple-600/20 to-cyan-400/30 animate-spin" style={{ animationDuration: '20s' }} />
                  <div className="w-full h-full rounded-full bg-slate-900 flex flex-col items-center justify-center p-2 relative z-10 text-center border border-slate-700">
                    <div className="p-2 rounded-xl bg-gradient-to-tr from-indigo-500 to-cyan-400 text-white mb-1 shadow-md">
                      <Cpu className="w-6 h-6" />
                    </div>
                    <span className="text-xs sm:text-sm font-black tracking-wider text-white">PLEXAAR</span>
                    <span className="text-[9px] font-bold text-cyan-400 tracking-widest uppercase">CORE</span>
                  </div>
                </div>
              </div>
              ...
              >
                <div className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-slate-900 border border-cyan-500/50 shadow-xl shadow-cyan-500/20 hover:border-cyan-400 transition-all">
                  <div className="p-2 rounded-xl bg-cyan-500/20 text-cyan-400 border border-cyan-500/30">
                    <MessageSquare className="w-5 h-5" />
                  </div>
                  <div className="text-left hidden sm:block">
                    <div className="text-xs font-black text-white group-hover:text-cyan-300">BUZZCOM</div>
              ...
              >
                <div className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-slate-900 border border-purple-500/50 shadow-xl shadow-purple-500/20 hover:border-purple-400 transition-all">
                  <div className="p-2 rounded-xl bg-purple-500/20 text-purple-400 border border-purple-500/30">
                    <Calendar className="w-5 h-5" />
                  </div>
                  <div className="text-left hidden sm:block">
                    <div className="text-xs font-black text-white group-hover:text-purple-300">CALENDEX</div>
              ...
              >
                <div className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-slate-900 border border-blue-500/50 shadow-xl shadow-blue-500/20 hover:border-blue-400 transition-all">
                  <div className="p-2 rounded-xl bg-blue-500/20 text-blue-400 border border-blue-500/30">
                    <Users className="w-5 h-5" />
                  </div>
                  <div className="text-left hidden sm:block">
                    <div className="text-xs font-black text-white group-hover:text-blue-300">iREACH</div>
              ...
              >
                <div className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-slate-900 border border-emerald-500/50 shadow-xl shadow-emerald-500/20 hover:border-emerald-400 transition-all">
                  <div className="p-2 rounded-xl bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">
                    <TrendingUp className="w-5 h-5" />
                  </div>
                  <div className="text-left hidden sm:block">
                    <div className="text-xs font-black text-white group-hover:text-emerald-300">ONDAL</div>
              ...
              >
                <div className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-slate-900 border border-pink-500/50 shadow-xl shadow-pink-500/20 hover:border-pink-400 transition-all">
                  <div className="p-2 rounded-xl bg-pink-500/20 text-pink-400 border border-pink-500/30">
                    <Megaphone className="w-5 h-5" />
                  </div>
                  <div className="text-left hidden sm:block">
                    <div className="text-xs font-black text-white group-hover:text-pink-300">SALEXPLEX</div>
            ...
            <div className="mt-6 p-4 rounded-2xl bg-slate-900/90 border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-xl bg-gradient-to-r from-indigo-500 to-cyan-500 text-white">
                  <Zap className="w-4 h-4 animate-bounce" />
                </div>
                <div>
                  <div className="text-xs font-bold text-white flex items-center gap-2">
                    <span>Ecosystem Data Stream:</span>
                    <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-indigo-500/20 text-cyan-300 border border-indigo-500/30">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-4 py-2 rounded-xl text-xs font-bold bg-slate-800 hover:bg-slate-700 text-cyan-300 border border-slate-700 transition-all flex items-center gap-1.5"
                >
        ...
        <div className="mt-16 max-w-6xl mx-auto">
          <div className="text-center mb-8">
            <span className="text-xs font-extrabold uppercase tracking-widest text-cyan-400">PRODUCT BACKGROUND SHOWCASE</span>
            ...
            {PLEXAAR_PRODUCTS.map((prod) => {
              ...
              return (
                <div 
                  ...
                  className="group relative rounded-2xl overflow-hidden border border-slate-800 bg-slate-900/90 hover:border-cyan-500/50 transition-all duration-300 cursor-pointer shadow-lg hover:shadow-cyan-500/20 flex flex-col justify-between h-56"
                >
                  {/* Background Image */}
                  <div 
                  ...
                  <div className="relative z-10 p-5 flex items-start justify-between">
                    <span className="px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-slate-950/80 border border-slate-700 text-cyan-300">
                      {prod.category}
                    </span>
